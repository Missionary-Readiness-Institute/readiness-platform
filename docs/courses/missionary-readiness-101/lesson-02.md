# Lesson 02: Readiness Areas & Risk Awareness

## Key Idea

Readiness is a whole-task habit. Spiritual, relational, cultural, safety, and practical preparation each reduce different kinds of risk.

## Lesson Goal

Map the main readiness areas and recognize the risks when one area is ignored.

## What This Lesson Teaches

- Readiness is a whole-task habit. Spiritual, relational, cultural, safety, and practical preparation each reduce different kinds of risk.
- How the idea connects to the course output.
- A practical action you can complete immediately.

## Practical Action

Write one sentence for each readiness area and note one immediate question you need to answer in each area.

<div class="skill-check"
     data-question="True or false: readiness only means organizing documents and sending support letters."
     data-options='["True", "False"]'
     data-answer="1"
     data-explanation="Readiness includes Spiritual Readiness and Resilience, humility, relationships, safety, and practical planning, not just paperwork.">
</div>

## Lesson Summary

This lesson reinforces the main idea in the context of the course output. Use it to build the next part of your plan.

<div class="course-nav">
  <a class="mri-button secondary" href="lesson-01.md">Back</a>
  <a class="mri-button primary" href="lesson-03.md">Next Lesson</a>
</div>
